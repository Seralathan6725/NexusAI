...
"""
    ...
    def _parse_generated_code(self, code_text: str) -> Dict:
        ...
        global_match = re.search(r'GLOBAL LABEL TABLE(.*?)(?=PROGRAM BLOCK|FUNCTION(?!\s+BLOCK)|FUNCTION BLOCK|$)', 
        ...
        program_pattern = r'PROGRAM BLOCK\s*\n(.*?)(?=(?:PROGRAM BLOCK|FUNCTION(?!\s+BLOCK)|FUNCTION BLOCK|$))'
        for match in re.finditer(program_pattern, code_text, re.DOTALL | re.IGNORECASE):
            block_content = match.group(1)
            program_block = self._parse_program_block(block_content)
            if program_block:
                result['program_blocks'].append(program_block)
                logger.info(f"Parsed program block: {program_block.get('name', 'Unknown')}")
        
        # Extract all FUNCTIONS (but not FUNCTION BLOCKS)
        function_pattern = r'(?<!FUNCTION )FUNCTION\s*\n(.*?)(?=(?:PROGRAM BLOCK|(?<!FUNCTION )FUNCTION\s|FUNCTION BLOCK|$))'
